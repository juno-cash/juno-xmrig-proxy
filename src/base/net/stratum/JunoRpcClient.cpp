...
void xmrig::JunoRpcClient::submitBlock(const JobResult &result)
{
    ...
    // Nonce (32 bytes) - parse from hex string in proxy's JobResult
    // Proxy stores nonce as hex string (64 chars for 32 bytes)
    uint8_t nonceBytes[32] = {0};
    if (result.nonce && strlen(result.nonce) >= 64) {
        Cvt::fromHex(nonceBytes, 32, result.nonce, 64);
    }
    block.insert(block.end(), nonceBytes, nonceBytes + 32);

    // Solution (varint length + 32 bytes hash)
    // For RandomX, solution is 32 bytes (the hash)
    // Parse result hash from hex string
    uint8_t resultHash[32] = {0};
    if (result.result && strlen(result.result) >= 64) {
        Cvt::fromHex(resultHash, 32, result.result, 64);
    }
    block.push_back(32);  // CompactSize varint for 32
    block.insert(block.end(), resultHash, resultHash + 32);
    ...
    m_results[m_sequence] = SubmitResult(m_sequence, result.diff, result.actualDiff(), 0, 0);
...
bool xmrig::JunoRpcClient::parseBlockTemplate(const rapidjson::Value &result)
{
    ...
    // Set job blob from the 140-byte header (convert to hex string for proxy)
    char headerHex[281] = {0};
    Cvt::toHex(headerHex, sizeof(headerHex), header, sizeof(header));
    job.setBlob(headerHex);

    // Set seed hash and height
    job.setSeedHash(seedHashHex.data());
    job.setHeight(height);

    // Compute difficulty from compact "bits" field and set on job
    {
        uint32_t compact = m_headerBits;
        int exponent = (compact >> 24) & 0xff;
        uint32_t mantissa = compact & 0x007fffff;
        int mantissaPos = exponent - 3;
        ...
        // Convert target to difficulty and set
        uint64_t difficulty = (target64 > 0) ? Job::toDiff(target64) : 1;
